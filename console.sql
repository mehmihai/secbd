...
SELECT USERNAME,TIMESTAMP,ACTION_NAME FROM dba_audit_trail WHERE USERNAME='SYSTEM';

--object level audit
AUDIT INSERT ON USERS;
INSERT INTO USERS (ID, EMAIL, PASSWORD_HASH ) VALUES(6 , 'asd6@asd.com', '231566');
SELECT object_name,object_type,ins,sel FROM dba_obj_audit_opts WHERE owner='SYS';

--priviledge audit
AUDIT CREATE ANY TABLE BY SYSTEM;
CREATE TABLE PRIVILEGE_TEST(TEST VARCHAR2(10)); -- RUN AS SYSTEM
SELECT username,timestamp,action_name,sys_privilege,priv_used FROM dba_audit_trail WHERE username='SYSTEM';
...
CREATE OR REPLACE TRIGGER AUDIT_ORDER_CHANGES
    BEFORE DELETE ON SYSTEM.ORDERS
    FOR EACH ROW
BEGIN
    INSERT INTO AUDIT_ORDERS(ID,RESTAURANT_ID,CUSTOMER_ID,DRIVER_ID,PROMO_ID,DATE_CREATED,DATE_COMPLETED,TOTAL_ORDER_PRICE,CHANGE_TYPE,USER_NAME,TIME)
    VALUES (:OLD.ID, :OLD.RESTAURANT_ID,:OLD.CUSTOMER_ID,:OLD.DRIVER_ID,:OLD.PROMO_ID,:OLD.DATE_CREATED,:OLD.DATE_COMPLETED,:OLD.TOTAL_ORDER_PRICE,'D',USER,SYSDATE);
END;

--disable constraints for test
ALTER TABLE DRIVER_RATING DISABLE CONSTRAINT DRIVER_RATING_ORDER_ID_FK;
ALTER TABLE ORDER_PRODUCTS DISABLE CONSTRAINT ORDER_PRODUCTS_ORDER_ID_FK;
...
--enable constraints
ALTER TABLE DRIVER_RATING ENABLE CONSTRAINT DRIVER_RATING_ORDER_ID_FK;
ALTER TABLE ORDER_PRODUCTS ENABLE CONSTRAINT ORDER_PRODUCTS_ORDER_ID_FK;
...
CREATE OR REPLACE PACKAGE BODY CUSTOMER_MASK AS
    PROCEDURE MASK AS
            CURSOR C_MASK IS SELECT ID, name, phone FROM CUSTOMER;
